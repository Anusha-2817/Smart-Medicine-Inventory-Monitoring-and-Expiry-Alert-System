...
export const generateLowStockAlerts = async () => {
  ...
  for (const medicine of medicines) {
    const totalStock = medicine.inventoryBatches.reduce(
      (sum, batch) => sum + batch.quantity,
      0,
    );

    if (totalStock <= medicine.reorderThreshold) {
...
export const generateExpiryAlerts = async () => {
